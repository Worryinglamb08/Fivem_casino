...
AddEventHandler('program-blackjack:pickWinner', function()
    ...
    if player1result >= 21 and player2result >= 21 then
        TriggerClientEvent('esx:showNotification', player1.source, '~y~Won~s~!')
        TriggerClientEvent('esx:showNotification', player2.source, '~y~Won~s~!')
        TriggerClientEvent('esx:showNotification', croupier.source, '~y~Won~s~!')
        if player1black then
            player1.addAccountMoney('black_money', bet)
        else
            player1.addMoney(bet)
